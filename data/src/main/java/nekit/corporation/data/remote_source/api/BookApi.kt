...
interface BookApi {
    ...
    @GET("api/books")
    suspend fun getBooksByGenre(@Query("filters[genres][id][\$eq]") genre: Long): BooksDto

    @GET("api/books")
    suspend fun getBooksByAuthor(@Query("filters[authors][id][\$eq]") authors: Long): BooksDto
